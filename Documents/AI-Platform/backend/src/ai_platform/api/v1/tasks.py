...
def list_tasks(
    ...
    module_filter: Optional[str] = Query(None, description="Filtrar por módulo"),
...
def get_task(
    ...
    if not task:
        raise HTTPException(
            ...
            detail="Tarea no encontrada o no tienes acceso a ella"
        )
...
def delete_task(
    ...
    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Tarea no encontrada o está en ejecución"
        )
