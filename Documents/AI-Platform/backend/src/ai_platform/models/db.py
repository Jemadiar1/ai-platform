...
from sqlalchemy import (
    Column, String, UUID, DateTime, Integer, JSON, Text, Boolean, Float, func
)
...
class Tenant(Base):
    ...
    id = Column(
        ...
        primary_key=True,
        default=uuid4,
        doc="UUID v4 único del tenant"
    )
    name = Column(
        String(255),
        nullable=False,
        doc="Nombre de la empresa/cliente"
    )
    slug = Column(
        String(100),
        unique=True,
        nullable=False,
        index=True,
        doc="Identificador URL único (ej: 'mi-empresa')"
    )
    plan = Column(
        String(50),
        nullable=False,
        default="free",
        doc="Plan actual: free, starter, pro, enterprise"
    )
    billing_email = Column(
        String(255),
        doc="Email para facturación"
    )
    clerk_user_id = Column(
        String(255),
        unique=True,
        nullable=True,
        doc="ID externo del usuario en Clerk (clerk.com)"
    )
    settings = Column(
        JSON,
        default=dict,
        doc="Configuración personalizada: idioma, zona horaria, integraciones"
    )
    is_active = Column(
        Boolean,
        default=True,
        doc="Estado del tenant (activo/inactivo)"
    )
...
class User(Base):
    ...
    tenant_id = Column(
        ...
        nullable=False,
        index=True,
        doc="Tenant al que pertenece este usuario"
    )
    clerk_user_id = Column(
        String(255),
        nullable=False,
        doc="ID del usuario en Clerk"
    )
...
class Task(Base):
    ...
    module = Column(
        String(50),
        nullable=False,
        doc="Módulo: ai-connect, ai-social, ai-content, etc."
    )
    status = Column(
        String(20),
        nullable=False,
        default="pending",
        index=True,
        doc="pending, running, completed, failed, retrying"
    )
...
class UsageEvent(Base):
    ...
    task_id = Column(
        ...
        nullable=True,
        index=True,
        doc="Tarea asociada a este evento de uso"
    )
...
class AgentMemory(Base):
    ...
    type = Column(
        String(20),
        nullable=False,
        doc="short_term (sesión) o long_term (semántica)"
    )
...
class Session(Base):
    ...
    parent_session_id = Column(
        ...
        nullable=True,
        index=True,
        doc="ID de sesión padre (para tracking de subagentes)"
    )
